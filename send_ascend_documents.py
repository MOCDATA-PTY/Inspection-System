#!/usr/bin/env python3
"""
Script to download and send Ascend Investment Holding documents
"""
...
def sanitize_client_name(client_name):
    ...
    sanitized = re.sub(r'[^a-zA-Z0-9_]', '', client_name)
    # Replace multiple underscores with single
    sanitized = re.sub(r'_+', '_', sanitized)
    # Remove leading/trailing underscores
    return sanitized.strip('_')

def download_client_documents(client_name, inspection_date, account_code):
    """Download compliance documents for specific client."""
    print(f"🔍 Looking for documents for: {client_name}")
    ...
    drive_files = drive_service.list_files(compliance_service.DRIVE_FOLDER_ID)
    ...
    client_folder = sanitize_client_name(client_name)
    ...
    for inspection in inspections:
        print(f"🔍 Processing inspection {inspection.remote_id} - Commodity: {inspection.commodity}")
        ...
        for file_info in drive_files:
            ...
            if (account_code in file_name and 
...
def create_document_zip(client_name, inspection_date, base_path):
    ...
    with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        ...
        if os.path.exists(compliance_path):
            for root, dirs, files in os.walk(compliance_path):
                for file in files:
        ...
        # Add other document types if they exist
        for doc_type in ['rfi', 'invoice', 'lab', 'retest']:
            doc_path = os.path.join(base_path, doc_type)
            if os.path.exists(doc_path):
                for file in os.listdir(doc_path):
                    file_path = os.path.join(doc_path, file)
                    if os.path.isfile(file_path):
                        arcname = f"{doc_type.upper()}/{file}"
                        zipf.write(file_path, arcname)
                        print(f"📎 Added to zip: {arcname}")
...
Please find attached the documentation pertaining to the inspection(s) conducted at Ascend Investment Holding (PTY) LTD/ Xing Xing facility on 09/03/2025.

Inspection Overview:
- Client: Ascend Investment Holding (PTY) LTD/ Xing Xing
- Date Inspected: 09/03/2025
...
def main():
    """Main function to download and send documents."""
    client_name = "Ascend Investment Holding (PTY) LTD/ Xing Xing"
    inspection_date = datetime(2025, 3, 9)  # 09/03/25
    account_code = "FA-IND-EGG-NA-0014"
    ...
    # Download documents
    result = download_client_documents(client_name, inspection_date, account_code)
    
    if not result:
        print("❌ No documents found or error occurred")
        return
    
    downloaded_files, base_path = result
    ...
    # Create zip file
    zip_path = create_document_zip(client_name, inspection_date, base_path)

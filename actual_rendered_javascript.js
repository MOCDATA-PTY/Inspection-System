...
// === RENDERED SCRIPT BLOCK 2 ===
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Automatically update View Files button colors on page load with a delay
            setTimeout(() => {
        ...
        // Note: Expand buttons use event delegation below, no individual listeners needed
        
        // Add listener for pagination changes to re-run color update
        const paginationLinks = document.querySelectorAll('.pagination a');
        paginationLinks.forEach(link => {
            link.addEventListener('click', function() {
                // Wait for new page to load, then update colors and expand buttons
                setTimeout(() => {
                    console.log('📄 [FRONTEND] Page changed, updating View Files button colors...');
                    updateAllViewFilesButtonColors();
                    // Event delegation handles expand buttons automatically
                }, 1000);
            });
        });
        ...
        const observer = new MutationObserver(function(mutations) {
            ...
            mutations.forEach(function(mutation) {
                if (mutation.type === 'childList') {
                    // Check if View Files buttons were added
                    const addedNodes = Array.from(mutation.addedNodes);
                    addedNodes.forEach(node => {
                        if (node.nodeType === Node.ELEMENT_NODE) {
                            const viewFilesButtons = node.querySelectorAll ? node.querySelectorAll('.btn-view-files, button[class*="btn-view-files"]') : [];
                            if (viewFilesButtons.length > 0 || (node.classList && node.classList.contains('btn-view-files'))) {
                                shouldUpdate = true;
                            }
                        }
                    });
            ...
            if (shouldUpdate) {
                console.log('🔄 [FRONTEND] New View Files buttons detected, updating colors...');
                setTimeout(() => {
                    updateAllViewFilesButtonColors();
                    // Event delegation handles expand buttons automatically
                }, 500);
        ...
        // Event delegation for expand buttons (primary method)
        document.addEventListener('click', function(e) {
            if (e.target.closest('.expand-btn')) {
                ...
                console.log('Expand button clicked via delegation for group:', groupId);
                toggleGroup(e, groupId);
            }
        });
        
        // Event delegation handles expand buttons automatically, no window load setup needed
            ...
            fileInput.onchange = function(e) {
                ...
                if (file) {
                    ...
                    const csrfToken = document.querySelector('[name=csrfmiddlewaretoken]')?.value || 
                                     'zaoHve34AJCsLD6B2pIw6iWcBJSNwnbSvrtRpsrQbgCOkvbDKxZvXvr6sSgBR6PN';
...
// === END RENDERED SCRIPT BLOCK 2 ===

// === RENDERED SCRIPT BLOCK 3 ===
        ...
        function syncInspectionsFromShipmentList() {
            ...
            const csrfToken = document.querySelector('[name=csrfmiddlewaretoken]')?.value || 'zaoHve34AJCsLD6B2pIw6iWcBJSNwnbSvrtRpsrQbgCOkvbDKxZvXvr6sSgBR6PN';

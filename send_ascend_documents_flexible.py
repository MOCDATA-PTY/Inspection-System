...
def find_client_inspections(client_name, account_code):
    """Find inspections for the client with flexible matching."""
    print(f"🔍 Searching for inspections for: {client_name}")
    print(f"🏢 Account Code: {account_code}")
    
    # Try different search patterns
    search_patterns = [
        {'client_name__icontains': 'Ascend'},
        {'client_name__icontains': 'Xing'},
        {'account_code': account_code},
        {'account_code__icontains': 'EGG'},
    ]
    
    all_inspections = []
    
    for pattern in search_patterns:
        inspections = FoodSafetyAgencyInspection.objects.filter(**pattern)
        if inspections.exists():
            print(f"✅ Found {inspections.count()} inspections with pattern: {pattern}")
            for inspection in inspections:
                print(f"  ID: {inspection.remote_id}, Client: {inspection.client_name}, Date: {inspection.date_of_inspection}, Commodity: {inspection.commodity}")
                all_inspections.append(inspection)
    
    return all_inspections

def download_available_documents(inspections):
    """Download any available compliance documents."""
    if not inspections:
    ...
    for inspection in inspections:
        ...
        for file_info in drive_files:
            ...
            if (inspection.account_code in file_name and 
...
def main():
    """Main function to find, download and send documents."""
    ...
    print("🚀 Starting document search and email process...")
    print(f"📧 Target email: {recipient_email}")
    
    # Find inspections
    inspections = find_client_inspections(client_name, account_code)
    
    if not inspections:
        print("❌ No inspections found for this client")
        return
    
    # Download documents
    downloaded_files, base_paths = download_available_documents(inspections)

...
# -*- coding: utf-8 -*-
"""
Migrate REMAINING files from old path structure to new inspection-based structure
Handles files without dates in filenames by using:
1. File modification date
2. Most recent inspection for client
...
def find_inspection_by_client(client_name):
    """Find the most recent inspection for a client"""

    # Clean client name for matching
    clean_client = client_name.replace('_', ' ').replace('mobile ', '').strip()

    print(f"    🔍 Looking for inspection: {clean_client}")
    ...
    if inspections.exists():
        ...
        print(f"    ✅ Found exact match: {inspection.remote_id} - {inspection.client_name} on {inspection.date_of_inspection}")
        return inspection
    ...
    if inspections.exists():
        inspection = inspections.first()
        print(f"    ✅ Found partial match: {inspection.remote_id} - {inspection.client_name} on {inspection.date_of_inspection}")
        return inspection

    # Try searching for parts of the name
    name_parts = clean_client.split()
    if len(name_parts) >= 2:
        for part in name_parts:
            if len(part) >= 4:  # Only search for meaningful words
                inspections = FoodSafetyAgencyInspection.objects.filter(
                    client_name__icontains=part
                ).order_by('-date_of_inspection')

                if inspections.exists():
                    inspection = inspections.first()
                    print(f"    ⚠️  Found match by keyword '{part}': {inspection.remote_id} - {inspection.client_name} on {inspection.date_of_inspection}")
                    return inspection
...
def migrate_remaining_files(dry_run=True):
    """Migrate remaining files from old structure to new structure"""

    media_root = settings.MEDIA_ROOT
    inspection_root = os.path.join(media_root, 'inspection')

    print(f"\n{'='*100}")
    print(f"MIGRATING REMAINING FILES FROM OLD STRUCTURE TO NEW STRUCTURE")
    print(f"{'='*100}")
    ...
    # Define old structure folders to look for
    ...
    for year in os.listdir(inspection_root):
        ...
        for month in os.listdir(year_path):
            ...
            for client_dir in os.listdir(month_path):
                ...
                # Find inspection record by client name only
                inspection = find_inspection_by_client(client_dir)
...
if __name__ == "__main__":
    print("\n" + "="*100)
    print("REMAINING FILES MIGRATION TOOL")
    print("="*100)
    ...
    try:
        migrate_remaining_files(dry_run=dry_run)
    except Exception as e:
        print(f"\n❌ ERROR: {str(e)}")
        import traceback
        traceback.print_exc()

...
try:
    ...
    # Fallback FSA query (comprehensive inspection data) - LAST 6 MONTHS ONLY
    FSA_INSPECTION_QUERY = '''
        SELECT DISTINCT 'POULTRY' as Commodity, DateOfInspection, StartOfInspection, EndOfInspection, InspectionLocationTypeID, IsDirectionPresentForthisInspection, InspectorId, gps.Latitude AS Latitude, gps.Longitude AS Longitude, NULL AS IsSampleTaken, NULL AS InspectionTravelDistanceKm, [AFS].[dbo].[PoultryQuidInspectionRecordTypes].Id as Id, clt.Name as Client FROM AFS.dbo.PoultryQuidInspectionRecordTypes JOIN AFS.dbo.GPSInspectionLocationRecords gps ON gps.PoultryQuidInspectionRecordId = [AFS].[dbo].PoultryQuidInspectionRecordTypes.Id join AFS.dbo.Clients clt on clt.Id = [AFS].[dbo].PoultryQuidInspectionRecordTypes.ClientId where AFS.dbo.PoultryQuidInspectionRecordTypes.IsActive = 1 AND DateOfInspection >= DATEADD(MONTH, -6, GETDATE()) 
        UNION 
        SELECT DISTINCT 'POULTRY' as Commodity, DateOfInspection, StartOfInspection, EndOfInspection, InspectionLocationTypeID, IsDirectionPresentForthisInspection, InspectorId, gps.Latitude AS Latitude, gps.Longitude AS Longitude, NULL AS IsSampleTaken, NULL AS InspectionTravelDistanceKm, [AFS].[dbo].[PoultryGradingInspectionRecordTypes].Id as Id, clt.Name as Client FROM AFS.dbo.PoultryGradingInspectionRecordTypes JOIN AFS.dbo.GPSInspectionLocationRecords gps ON gps.PoultryGradingClassInspectionRecordId = [AFS].[dbo].[PoultryGradingInspectionRecordTypes].Id join AFS.dbo.Clients clt on clt.Id = [AFS].[dbo].PoultryGradingInspectionRecordTypes.ClientId where AFS.dbo.[PoultryGradingInspectionRecordTypes].IsActive = 1 AND DateOfInspection >= DATEADD(MONTH, -6, GETDATE()) 
        UNION 
        SELECT DISTINCT 'POULTRY' as Commodity, DateOfInspection, StartOfInspection, EndOfInspection, InspectionLocationTypeID, IsDirectionPresentForthisInspection, InspectorId, gps.Latitude AS Latitude, gps.Longitude AS Longitude, NULL AS IsSampleTaken, NULL AS InspectionTravelDistanceKm, [AFS].[dbo].[PoultryLabelInspectionChecklistRecords].Id as Id, clt.Name as Client FROM AFS.dbo.PoultryLabelInspectionChecklistRecords JOIN AFS.dbo.GPSInspectionLocationRecords gps ON gps.PoultryLabelChecklistInspectionRecordId = [AFS].[dbo].[PoultryLabelInspectionChecklistRecords].Id join AFS.dbo.Clients clt on clt.Id = [AFS].[dbo].[PoultryLabelInspectionChecklistRecords].ClientId where AFS.dbo.[PoultryLabelInspectionChecklistRecords].IsActive = 1 AND DateOfInspection >= DATEADD(MONTH, -6, GETDATE()) 
        UNION 
        SELECT DISTINCT 'EGGS' as Commodity, DateOfInspection, StartOfInspection, EndOfInspection, InspectionLocationTypeID, IsDirectionPresentForInspection as IsDirectionPresentForthisInspection, InspectorId, gps.Latitude AS Latitude, gps.Longitude AS Longitude, NULL AS IsSampleTaken, NULL AS InspectionTravelDistanceKm, [AFS].[dbo].[PoultryEggInspectionRecords].Id as Id, clt.Name as Client FROM [AFS].[dbo].[PoultryEggInspectionRecords] JOIN AFS.dbo.GPSInspectionLocationRecords gps ON gps.PoultryEggInspectionRecordId = [AFS].[dbo].[PoultryEggInspectionRecords].Id join AFS.dbo.Clients clt on clt.Id = [AFS].[dbo].[PoultryEggInspectionRecords].ClientId where AFS.dbo.[PoultryEggInspectionRecords].IsActive = 1 AND DateOfInspection >= DATEADD(MONTH, -6, GETDATE()) 
        UNION 
        SELECT DISTINCT 'RAW' as Commodity, DateOfInspection, StartOfInspection, EndOfInspection, InspectionLocationTypeID, IsDirectionPresentForthisInspection, InspectorId, gps.Latitude AS Latitude, gps.Longitude AS Longitude, IsSampleTaken, InspectionTravelDistanceKm, [AFS].[dbo].[RawRMPInspectionRecordTypes].Id as Id, clt.Name as Client FROM [AFS].[dbo].[RawRMPInspectionRecordTypes] JOIN AFS.dbo.GPSInspectionLocationRecords gps ON gps.RawRMPInspectionRecordId = [AFS].[dbo].[RawRMPInspectionRecordTypes].Id JOIN AFS.dbo.RawRMPInspectedProductRecordTypes prod on prod.InspectionId = [AFS].[dbo].[RawRMPInspectionRecordTypes].Id join AFS.dbo.Clients clt on clt.Id = prod.ClientId where AFS.dbo.[RawRMPInspectionRecordTypes].IsActive = 1 AND DateOfInspection >= DATEADD(MONTH, -6, GETDATE()) 
        UNION 
        SELECT DISTINCT 'PMP' as Commodity, DateOfInspection, StartOfInspection, EndOfInspection, InspectionLocationTypeID, IsDirectionPresentForthisInspection, InspectorId, gps.Latitude AS Latitude, gps.Longitude AS Longitude, IsSampleTaken, NULL AS InspectionTravelDistanceKm, [AFS].[dbo].[PMPInspectionRecordTypes].Id as Id, clt.Name as Client FROM [AFS].[dbo].[PMPInspectionRecordTypes] join AFS.dbo.GPSInspectionLocationRecords gps on gps.PMPInspectionRecordId = [AFS].[dbo].[PMPInspectionRecordTypes].Id JOIN AFS.dbo.PMPInspectedProductRecordTypes prod on prod.InspectionId = [AFS].[dbo].[PMPInspectionRecordTypes].Id join AFS.dbo.Clients clt on clt.Id = prod.ClientId where AFS.dbo.[PMPInspectionRecordTypes].IsActive = 1 AND DateOfInspection >= DATEADD(MONTH, -6, GETDATE())
    '''

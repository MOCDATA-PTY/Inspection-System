...
def test_client_name_handling():
    ...
    for client_name in test_clients:
        ...
        if not match_folder_sanitized and not match_folder_spaces:
            print(f"  ❌ MISMATCH! Upload creates '{folder_name}' but detection looks for '{sanitized_name}'")
        else:
            print(f"  ✅ MATCH! Detection will find the folder")
...
def test_file_path_simulation():
    ...
    for i, path in enumerate(detection_paths, 1):
        print(f"  {i}. {path}")
        match = (path.replace('/', '') == upload_path.replace('/', '').replace('rfi', ''))

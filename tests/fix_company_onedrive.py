#!/usr/bin/env python3
"""
Fix OneDrive for Company Account - Single Tenant
"""
...
def get_company_tenant_id():
    """Get the tenant ID for Food Safety Agency (Pty) Ltd."""
    # Since it's a single tenant app, we need to find the tenant ID
    # Let's try to discover it using the company domain
    
    # First, let's try with the company domain
    company_domain = "fsa-pty.co.za"
    
    # Try to get tenant info
    try:
        # Use the company domain to get tenant info
    ...
    # If that doesn't work, we'll need to use the common endpoint
    # but with the correct tenant-specific token endpoint
    return "common"

def authenticate_single_tenant():
    """Authenticate with single tenant company account."""
    print("🏢 ONEDRIVE SINGLE TENANT AUTHENTICATION")
    print("=" * 50)
    ...
    # Use tenant-specific endpoint for single tenant
    if tenant_id == "common":
        ...
        token_base_url = f"https://login.microsoftonline.com/{tenant_id}/oauth2/v2.0/token"
    ...
    try:
        response = requests.post(token_base_url, data=token_data)
...
if __name__ == "__main__":
    success = authenticate_single_tenant()
    if success:
        print("\n🎉 Single tenant OneDrive authentication successful!")
    else:
        print("\n❌ Single tenant OneDrive authentication failed")

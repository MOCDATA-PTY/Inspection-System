#!/usr/bin/env python3
"""
Simple test script to verify email functions are working correctly.
"""
...
def test_email_functions_simple():
    """Simple test of email functions."""
    ...
    # Test 1: Check if main functions are defined
    print("\n📋 Test 1: Main Function Definitions")
    
    main_functions = [
        'window.updateGroupAdditionalEmails = debounce(function(input)',
    ...
    # Check for basic script tag balance
    ...
    # Test 4: Check if functions are defined early enough
    print("\n📋 Test 4: Function Definition Order")
    ...
    for i, line in enumerate(lines):
        if 'window.updateGroupAdditionalEmails = debounce(function(input)' in line:
            main_def_line = i + 1
        if 'onchange="updateGroupAdditionalEmails(this)"' in line:
            html_call_line = i + 1
            break
    
    if main_def_line and html_call_line:
        if main_def_line < html_call_line:
            print(f"✅ Functions defined before HTML calls (line {main_def_line} < {html_call_line})")
        else:
            print(f"❌ Functions defined after HTML calls (line {main_def_line} > {html_call_line})")
            syntax_ok = False
    else:
        print("❌ Could not determine function definition order")
        syntax_ok = False
    ...
    tests = [
        ("Main Function Definitions", all_defined),
...
def main():
    ...
    try:
        result = test_email_functions_simple()
        if result:
